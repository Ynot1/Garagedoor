...
extern bool GarageDoorState;     //   LOW = door open   HIGH = Door Closed
  ...
  if(request.indexOf("GetBinaryState") >= 0) {
    ...
    switchStatus = !GarageDoorState; 
      ...
      if (GarageDoorState == LOW) {
              //Serial.println("GarageDoor State is LOW (door open))");
     }  

          if (GarageDoorState == HIGH) {
              //Serial.println("GarageDoor State is HIGH (door closed))");
     } 
